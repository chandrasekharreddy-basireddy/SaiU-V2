...
export function reconcileReminderScope(){const current=scope();const valid=new Set();for(const r of read()){if(!r?.key||!Number.isFinite(r.fireAt))continue;if(r.scope&&r.scope!==current){clearTimer(r.key);continue}valid.add(r.key)}for(const key of [...timers.keys()])if(!valid.has(key))clearTimer(key);write(read().filter(r=>!r?.scope||r.scope===current));return valid.size}
export function recoverDueReminders(){const current=Date.now();reconcileReminderScope();const active=[];for(const r of read()){if(!r?.key||!Number.isFinite(r.fireAt))continue;if(r.fireAt<=current){void notify('Upcoming class',`${r.course} was due ${r.minutes} minutes before class — ${r.room||'room TBD'}`,`class-${r.key}`);continue}active.push(r);scheduleStored(r)}write(active);return active.length}
export function remindBeforeClass(c,minutes=10,now=new Date()){if(!c)return null;reconcileReminderScope();const target=targetDate(c,now);if(!target)return null;const lead=Math.max(1,Math.min(120,Number(minutes)||10)),fireAt=target.getTime()-lead*60000;if(fireAt<=now.getTime()||fireAt-now.getTime()>7*86400000)return null;const key=`${classKey(c)}:${target.getTime()}:${lead}`;const currentScope=scope();const prefix=`${classKey(c)}:`;const reminders=read().filter(r=>(r.scope||'')===currentScope&&!String(r.key||'').startsWith(prefix));reminders.push({key,fireAt,minutes:lead,course:String(c.course||'Class').slice(0,200),room:String(c.room||'').slice(0,120),scope:currentScope});write(reminders);scheduleStored(reminders.at(-1));return key}
