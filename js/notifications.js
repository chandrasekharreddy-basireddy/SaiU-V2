...
export function recoverDueReminders(){const current=Date.now();const active=[];for(const r of read()){if(!r?.key||!Number.isFinite(r.fireAt))continue;if(r.fireAt<=current){void notify('Upcoming class',`${r.course} was due ${r.minutes} minutes before class — ${r.room||'room TBD'}`,`class-${r.key}`);continue}active.push(r);scheduleStored(r)}write(active);return active.length}
export function remindBeforeClass(c,minutes=10,now=new Date()){if(!c)return null;const target=targetDate(c,now);if(!target)return null;const lead=Math.max(1,Math.min(120,Number(minutes)||10));const fireAt=target.getTime()-lead*60000;if(fireAt<=now.getTime()||fireAt-now.getTime()>7*86400000)return null;const key=`${classKey(c)}:${target.getTime()}:${lead}`;const reminders=read().filter(r=>!String(r.key||'').startsWith(`${classKey(c)}:`));const reminder={key,fireAt,minutes:lead,course:String(c.course||'Class').slice(0,200),room:String(c.room||'').slice(0,120)};reminders.push(reminder);write(reminders);scheduleStored(reminder);return key}
