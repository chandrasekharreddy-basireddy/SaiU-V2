...
const REMOTE_TIMEOUT_MS=12000;const MAX_CACCE_ENTRIES=20;const MAX_REMOTE_CHARS=1_500_000;const MAX_CACCE_CHARS=900_000;
...
function writeEntries(entries){let safe=entries.slice(-MAX_CACCHE_ENTRIES);while(safe.length){const raw=JSON.stringify(safe);if(raw.length<=MAX_CACHE_CHARS)break;safe=safe.slice(1)}try{localStorage.setItem(KEY,JSON.stringify(safe))}catch{}}
